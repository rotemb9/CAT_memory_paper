function probe(subjectID, order, mainPath, isMRI, sessionNum, block, numRun, numRunsPerBlock, trialsPerRun, total_num_blocks, use_eyetracker, debugging_mode)
% function probe(subjectID, order, mainPath, isMRI, sessionNum, block, numRun, numRunsPerBlock, trialsPerRun, total_num_blocks, use_eyetracker)
% = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
...
if numRun == 1
    if  block == 1 || block == 1+1*(sessionNum-1) % If this is the first run of the first block of this session, show instructions        
...
if isMRI % An fMRI experiment. Edit so that it would be good for the fmri experiment
    if numRun ~= numRunsPerBlock
        ...
        Screen('Flip',w);
        WaitSecs(3);
    elseif block~=total_num_blocks*sessionNum && numRun == numRunsPerBlock
        % this is the last run but not the last block
        CenterText(w,sprintf('In a moment we will complete another block of the same task') ,white,0,-170);
        Screen('Flip',w);
        WaitSecs(3);
    elseif block==total_num_blocks*sessionNum && numRun == numRunsPerBlock
        % this was the last run of the last block
        ...
        Screen('CloseAll');
        ShowCursor;
%     elseif block~=total_num_blocks*sessionNum && sessionNum > 1 && numRun == numRunsPerBlock
%         % this is the last block & it's a follow-up session and the last
%         %run of the block
%         CenterText(w,sprintf('This part is over') ,white,0,-270);
%         CenterText(w,sprintf('Questions?') ,white,0,-170);
%         Screen('Flip',w);
