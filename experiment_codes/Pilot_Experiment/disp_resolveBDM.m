...
% Open data from BDM`
BDM1_file = dir([outputPath '/' subjectID '_BDM' num2str(sessionNum) '*.txt']);
fid = fopen([outputPath '/' BDM1_file(end).name]);
BDMdata = textscan(fid, '%s\t%d\t%f\t%s\t%f\t%f\t%f' , 'HeaderLines', 1); %read in data as new matrix   
fclose(fid);
...
WaitSecs(1);
%%
fid2 = fopen([outputPath '/' subjectID '_BDM_resolve_session_' num2str(1) '.txt'],'a');

% save results

if str2double(chosenNumber) <= str2double(item_bid)
    % The subject buys the item for chosenNumber NIS
    fprintf(fid2, 'You may buy item %s for price %s  your bid was %s\n', item_chosen(1:end-4), chosenNumber, item_bid);
