function [Ladder1IN_new,Ladder2IN_new] = training(subjectID,order,mainPath,isMRI,runInd,total_num_runs_training,Ladder1IN,Ladder2IN,use_eyetracker,last_run, debugging_mode)
...
% - - - - - - - - - - - - - - -
file = dir([outputPath '/' subjectID '_stopGoList_allstim*.txt']);
...
all_runs = runInd:last_run;
% if debugging_mode
%     all_runs = runInd:runInd+3;
% end

for runNum = all_runs %this for loop allows all runs to be completed
    ...
    num_trials = length(shuff_trialType{runNum});
    if debugging_mode
        num_trials = 6;
    end
    
    for trialNum = 1:num_trials   % To cover all the items in one run.
