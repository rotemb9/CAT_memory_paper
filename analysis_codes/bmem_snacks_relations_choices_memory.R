...
if (sessionNum_probe == 2 & sessionNum_recognition == 2) {
  ...
  HV_accuracy_model=glmer(Outcome ~ 1 + accuracy_category + (1|subjectID),data=subset(probe_data,probe_data$PairType2=='High_Value' & !is.na(probe_data$accuracy_category)),na.action=na.omit,family=binomial)
  ...
  interaction_accuracy_model=glmer(Outcome ~ 1 + accuracy_category * PairType3 + (1 + PairType3.ind|subjectID),data=subset(probe_data,probe_data$PairType<=2 & !is.na(probe_data$accuracy_category)),na.action=na.omit,family=binomial)
...
plot_accuracy= ggplot(data=df_accuracy, aes(x=value_level, y=means, fill=category)) +
  ...
  theme(legend.position="none", legend.direction = "vertical", legend.title=element_blank()) + # position legend
  geom_text(aes(label = paste(considered_trials), y=8), size=12, position = position_dodge(0.7), angle = 90) +
  geom_text(aes(label = paste(asterisk), y = means + 14), size=12, position = position_dodge(0.7)) +
...
plot_RT= ggplot(data=df_RT_for_plot, aes(x=value_level, y=means, fill=item_type)) +
  ...
  theme(aspect.ratio=1.2, text = element_text(size=32)) + # font size
  geom_errorbar(aes(ymin=means-SEs, ymax=means+SEs), width=1/4, position=position_dodge(0.7)) +
  scale_y_continuous(expression(paste(Delta, "RT (NoGo recognition RT - Go recognition RT)")), expand = c(0,0), limit=c(-0.24,0.40), breaks=seq(-0.24,0.40,0.04)) + # define y axis properties
